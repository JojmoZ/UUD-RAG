...
class QdrantChecker:
    ...
    def _build_cache_file_map(self, cache_dir: str, cache_file_args: List[str] | None) -> Dict[str, str]:
        """Resolve cache file paths and derive chunker names from filenames."""
        if cache_file_args:
    ...
    def _reconstruct_chunk(self, chunk_dict: dict, chunk_type: str):
        """Rebuild chunk object from dict using known chunk types."""
        type_map = {
    ...
    def _load_missing_chunks_from_cache(self, cache_path: str, chunk_type_hint: str, missing_ids: Set[str]) -> Dict[str, object]:
        """Load and reconstruct only the missing chunks from a cache file."""
    ...
    def _insert_missing_chunks(self, cache_files_map: Dict[str, str], missing_by_chunker: Dict[str, Set[str]]):
        """Upsert missing chunks back into Qdrant from their cache files."""
        try:
            qdrant_db = Qdrant(
    ...
    def check_duplicates_in_qdrant(self, point_ids: Set[str], delete_duplicates: bool = False, cache_files_map: Dict[str, str] | None = None) -> Set[str]:
        """Check for duplicate point IDs in Qdrant and optionally delete them."""
        Logger.log(f"\n{'='*70}")
    ...
    def run_full_check(self, cache_dir: str = "./chunk_cache", save_report: bool = True, cache_files: List[str] | None = None, insert_missing: bool = False, delete_duplicates: bool = False, delete_extra: bool = False):
        """Run complete check, optionally delete duplicates/extra points, reinsert missing chunks, and generate report"""
        Logger.log(f"\n{'='*70}")
        ...
        if delete_duplicates and duplicate_ids:
            # Remove deleted IDs from in-memory set so comparison reflects current state
        ...
        # Optionally delete extra points (present in Qdrant but not in cache)
        if delete_extra:
        ...
        # Optionally insert missing chunks back into Qdrant
        if insert_missing:
